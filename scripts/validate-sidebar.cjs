...
const tocLinks = [
  ...tocContent.matchAll(/\/docs\/components\/([^'"\s\)},\]#]+)/g),
]
  .map((m) => m[1])
  .filter(Boolean)
  .filter((v, i, a) => a.indexOf(v) === i) // unique
...
console.log('\n✅ SUMMARY');
console.log(
  `Sidebar: ${valid.length}/${claimedLinks.length} valid (${claimedLinks.length > 0 ? Math.round((valid.length / claimedLinks.length) * 100) : 100}%)`
);
console.log(
  `TOC: ${tocValid.length}/${tocLinks.length} valid (${tocLinks.length > 0 ? Math.round((tocValid.length / tocLinks.length) * 100) : 100}%)`
);
