...
const sidebarClaims = validationData.sidebar?.claimed || [];
const brokenClaims = validationData.sidebar?.broken || [];
const linksToTest = sidebarClaims
  .filter((link) => !brokenClaims.includes(link))
...
async function testLink(url, index, total) {
  return new Promise((resolve) => {
    http
      ...
      .on('error', (err) => {
        console.log(
          `[${index + 1}/${total}] ❌ ${url} → ERROR: ${err.message}`
        );
        failed++;
        resolve();
      });
