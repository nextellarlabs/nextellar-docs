...
const afterLinks = [
  ...sidebarContent.matchAll(/\/docs\/components\/([^'"\s\)},\]]+)/g),
]
  .map((m) => m[1])
  .filter(Boolean);
...
console.log(
  `✅ Reduction: ${validation.total - afterLinks.length} links removed (${Math.round(((validation.total - afterLinks.length) / validation.total) * 100)}%)`
);
...
console.log(
  `  ${allValid ? '✅' : '❌'} All sidebar links point to existing files`
);
console.log(`  ${noBroken ? '✅' : '❌'} No broken links remain in sidebar`);
console.log(
  `  ${afterLinks.length > 0 ? '✅' : '❌'} Components section not empty (${afterLinks.length} items)`
);
...
if (allValid && noBroken && afterLinks.length > 0) {
  ...
  console.log('\n📝 SUMMARY FOR PR:');
  console.log(
    `- Removed ${validation.broken.length} broken component links (${Math.round((validation.broken.length / validation.total) * 100)}%)`
  );
  console.log(
    `- Validated ${afterLinks.length}/${afterLinks.length} remaining links work (100%)`
  );
