...
`--javascript` works with the `default`, `minimal`, and `defi` templates:
...
See [Scaffolding Templates](/docs/cli/templates) for what each template contains.
...
- **JavaScript configuration**: generated projects use `next.config.js`, JavaScript network configuration, and a JavaScript storage helper.
