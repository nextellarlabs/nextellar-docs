...
### 1a. Run the scaffold with `-c` / `--with-contracts`

The flag has a short form, `-c`, and a long form, `--with-contracts`. They do the same thing:

```bash
npx nextellar my-dapp --with-contracts
## or, equivalently
npx nextellar my-dapp -c
cd my-dapp
```

`--with-contracts` is an overlay, not a separate template, so you can combine it with any template or language flag:

```bash
## DeFi template + contracts
npx nextellar my-dapp --template defi -c

## JavaScript frontend + contracts (the contract itself is always Rust)
npx nextellar my-dapp --javascript -c
```

## 1b. What the CLI does

With the flag set, the scaffold runs its normal steps and then:

1. Copies the `contracts/` Rust workspace, including the `hello_world` contract, into the project root.
2. Adds `contracts:build` and `contracts:test` scripts to `package.json`.
3. Appends a `NEXT_PUBLIC_HELLO_WORLD_CONTRACT_ID` placeholder to `.env.example`.
4. Adds `src/lib/contracts/index.ts`, which exposes the contract ID to the frontend.

Nothing is compiled or deployed yet. Steps 3–6 below handle that.

### 1c. Generated project structure

Files added by `--with-contracts` are marked with `← contracts`. Everything else is the standard scaffold:

```text
my-dapp/
├── contracts/                          ← contracts
│   ├── Cargo.toml                      ← contracts  (Rust workspace manifest)
│   ├── README.md                       ← contracts  (build/test/deploy notes)
│   └── hello_world/                    ← contracts
│       ├── Cargo.toml                  ← contracts  (contract package manifest)
│       └── src/
│           └── lib.rs                  ← contracts  (the Soroban contract)
├── public/
├── src/
│   ├── app/
│   │   ├── layout.tsx
...
│   └── lib/
│       ├── contracts/
│       │   └── index.ts                ← contracts  (CONTRACTS map)
│       └── stellar-wallet-kit.ts
├── .env.example                        (+ NEXT_PUBLIC_HELLO_WORLD_CONTRACT_ID)
├── package.json                        (+ contracts:build, contracts:test)
├── next.config.ts
├── tsconfig.json
└── README.md
```

### 1d. Changes to existing files

`package.json` gains two scripts that run from inside `contracts/`:
...
`.env.example` gains a placeholder you'll replace after deploying:
...
## 1e. Verify the scaffold

```bash
ls contracts/hello_world/src          # lib.rs
npm run | grep contracts              # contracts:build, contracts:test
grep HELLO_WORLD .env.example         # NEXT_PUBLIC_HELLO_WORLD_CONTRACT_ID=...
```

If `contracts/` is missing, the flag wasn't passed. Re-run the scaffold into a new directory with `-c`.
