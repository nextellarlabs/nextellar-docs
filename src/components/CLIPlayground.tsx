...
export function CLIPlayground() {
    ...
    const [wallets, setWallets] = useState<string[]>(['freighter', 'albedo', 'lobstr']);
    const [customTimeout, setCustomTimeout] = useState(false);
    const [timeout, setTimeout] = useState('1200000');
    ...
    const generateCommand = () => {
        ...
        const walletsChanged = JSON.stringify(wallets.sort()) !== JSON.stringify(defaultWallets.sort());
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {(['npm', 'yarn', 'pnpm'] as const).map((pm) => (
                            <button
                                ...
                                className={`px-4 py-2 border rounded-md text-sm font-medium transition-colors ${packageManager === pm
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                        {availableWallets.map((wallet) => (
                            <button
                                ...
                                className={`px-3 py-2 border rounded-md text-sm transition-colors ${wallets.includes(wallet.id)
                                        ? 'bg-primary text-primary-foreground border-primary'
                                        : 'hover:bg-muted'
                                    }`}
                            >
